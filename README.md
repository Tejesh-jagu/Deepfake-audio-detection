...
4. Train the model (writes `best_model_fusion.h5` and plots under `eval_outputs/`):
   ```bash
   python -m src.train
   ```

5. **Enterprise UI (React + FastAPI)** — from the project root, two terminals:
   ```bash
   # Terminal A — API (loads best_model_fusion.h5)
   uvicorn backend.api:app --reload --host 127.0.0.1 --port 8000

   # Terminal B — frontend
   cd frontend && npm install && npm run dev
   ```
   Open `http://localhost:5173`. Set `VITE_API_URL` if the API is not on `http://127.0.0.1:8000`.

6. **Legacy Streamlit UI** (optional):
   ```bash
